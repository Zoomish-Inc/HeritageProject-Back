...
class Command(BaseCommand):
    help = 'Seed HeritageObject records from frontend mock data (one-time deploy helper).'
    ...
    def _import_object(self, item, frontend_base):
        ...
        if audio and (audio.get('tracks') or any(loc(audio.get(key))['ru'] or loc(audio.get(key))['uz'] for key in (
            'narratorLabel', 'transcript', 'atmosphereDescription', 'musicSuggestion'
        ))):
